-- USAGE 1 of 1: CommonsBase_Win32.Wenv@0.1.0
-- A UI rule that creates a Windows build environment ("wenv") on Unix (currently macOS only) with a
-- selected Windows toolchain package and optional mounted directories using the Wine emulator.
...
-- the `Wenv.Create` rule will wait for all `wineserver` transient processes on the macOS machine to finish before populating the
-- wenv.
-- 
-- DESIGN NOTES:
...
local M = {
    id = "CommonsBase_Win32.Wenv@0.1.0"
}
...
CommonsBase_Win32__Wenv__0_1_0 = {}
...
function uirules.Create(command, request)
    if command == "submit" then
        ...
        local commands = {
            ...
            { p.coreutilsexe, "rm", "-rf", p.wenv .. "/drive_c/opt/toolchain" },
            {
                p.coreutilsexe,
                "cp", "-r",
                "$(get-object " .. p.toolchain .. " -s Release.execution_abi.Windows_x86_64 -d :)",
                p.wenv .. "/drive_c/opt/toolchain"
            },
            ...
            -- PATH: add cygpath and the selected toolchain
            {
                "/bin/sh", p.enterwine, "--winehome", p.winehome, "--gnutls-libdir", p.gnutlslib, "--inotify-libdir", p.inotifylib, "--krb5-libdir", p.krb5lib,
                ...
                "/v", "Path",
                "/d", "C:\\opt\\cygpath;C:\\opt\\toolchain\\bin"
            },
        ...
        while k do
            local mount = CommonsBase_Win32__Wenv__0_1_0.parse_mount_option(v)
            ...
            assert(CommonsBase_Win32__Wenv__0_1_0.validate_absolutepath("The mount's [src]", mount.src))

            -- get Unix subpath on the M: drive
            local m_dst_subpath = CommonsBase_Win32__Wenv__0_1_0.m_drive_unix_subpath(mount.dst)

            -- create command to create parent directory, if any
            local m_dst_parent = CommonsBase_Win32__Wenv__0_1_0.unix_parent_dir(m_dst_subpath)
    ...
    elseif command == "ui" then
        ...
        print("For example, to open a Windows command prompt and inspect the selected toolchain, run:")
        print("  $ '" .. wenv .. "/bin/enter' cmd.exe")
        print("  Microsoft Windows 10.0.19045")
        print("")
        print("  Z:\\> dir C:\\opt\\toolchain\\bin")
        print("  Z:\\> set PATH")
        print("")
        print("  Z:\\> exit")
    end
...
function CommonsBase_Win32__Wenv__0_1_0.parse_mount_option(option)
    local mount = {}
    local pos = 1
...
function CommonsBase_Win32__Wenv__0_1_0.m_drive_unix_subpath(path)
